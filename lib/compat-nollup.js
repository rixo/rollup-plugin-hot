  ...
  const plugin = {
    ...
    transform(source, id) {
      ...
      const code = [
        [
          before,
          `import ______compatNollup__ from '${compatPath}';`,
          // 'ïmpørt_mêtä'.length === 'import.meta'.length
          `const ïmpørt_mêtä = { hot: ______compatNollup__(module) };`,
          after,
        ]
          .filter(Boolean)
          .join(' '),
        ...
        .join('\n')
        .replace(/\bimport.meta\b/g, 'ïmpørt_mêtä')
