...
const createServer = ({
  ...
  const writeBundle = (baseUrl, bundle, { sourcemap } = {}) => {
    console.time('server.writeBundle')
    ...
    console.timeEnd('server.writeBundle')
  }
  ...
  if (proxy) {
    ...
    for (const [base, target] of Object.entries(proxy)) {
      ...
      if (Array.isArray(target)) {
        handler = expressProxy(...target)
      } else {
        const url = new URL(target)

        // Forward base path
        // "/a/": "http://localhost/c/"
        // => /a/x/y => http://localhost/c/x/y
        ...
        if (
          ...
          handler = expressProxy(target, {
            proxyReqPathResolver(req) {
              return new URL(req.url.slice(1), url).href
            },
          })
        } else {
          handler = expressProxy(target)
