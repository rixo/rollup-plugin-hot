...
function autoCreateUnits({
  ...
  const warnCreate = warn === true || (warn && warn.create)
  const warnRecreate = recreate && (warn === true || (warn && warn.recreate))
  return {
...
const autoCreate = cfg => ({
  name: 'auto-create',
  options: applyAutoCreate(cfg),
})

module.exports = Object.assign(autoCreate, {
  applyAutoCreate,
})
