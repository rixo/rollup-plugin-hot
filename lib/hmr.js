      ...
      const entryLoaders = await Promise.all(
        Object.entries(bundle)
          ...
          .map(async ([key, chunk]) => {
            ...
            const fileName = path.resolve(dir, loaderFile || path.basename(key))
            return [
              fileName,
              {
                ...chunk,
                code,
                map: null,
                // TODO loaderFile is populated with output.file (works in
                // Routify example); keeping `fileName: key` works in Sapper
                // + Rollup... but why?
                fileName: loaderFile ? fileName : key,
              },
            ]
          })
      )
