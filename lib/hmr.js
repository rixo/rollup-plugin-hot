...
module.exports = (options = {}) => {
  const {
    ...
    passthroughModuleHot = false,
    nollup: compatNollup = false,
    public: publicDir = '',
    baseUrl = '/',
    inMemory = false,
    clearConsole = false,
  ...
  if (compatNollup) {
    log('Compat mode: only rewrites import.meta.hot to Nollup API')
    const compatPath = require.resolve('./compat-nollup.js')
    const { include = id => /\.(js|svelte)$/.test(id) } = { ...compatNollup }
    return {
      name: `${name} (compat mode: nollup)`,
      options: applyAutoCreate({ compatNollup: true, ...autoCreateOpt }),
      transform(code, id) {
        if (include(id) && id !== compatPath) {
          code =
            [
              `import __compatNollup from '${compatPath}';`,
              `const __importMetaHot = __compatNollup(module);`,
              `const __importMeta = { hot: __importMetaHot };`,
            ].join(' ') + code
          code = code.replace(/\bimport.meta.hot\b/g, '__importMetaHot')
  ...
  if (passthroughModuleHot) {
    log('Passthrough mode: only rewrites import.meta.hot to module.hot')
    return {
      name: `${name} (passthrough mode)`,
