...
const { log, posixify, readFile, runtimeFile } = require('./utils')
...
const createImportEntry = (chunk, { hotBundleUrl, inMemory }) => {
...
const createImportEntryPoints = (modules, { hotBundleUrl, inMemory }) =>
  modules
    .map(chunk => {
      const filename = chunk.fileName
      const url = hotBundleUrl + filename
...
const renderHotEntryLoader = ({
  ...
  doRenderHotLoader({
    serveRuntime,
    port,
    importEntryPoints: createImportEntry(chunk, { hotBundleUrl, inMemory }),
  })

// Writes a loader with `System.import(...)` for each entry point
const renderHotLoader = (
  bundle,
  {
    hotBundleUrl,
    inMemory,
    ...
    serveRuntime,
    runtimeScriptId,
    importEntryPoints = createImportEntryPoints(
      Object.values(bundle).filter(({ isEntry }) => isEntry),
      {
        hotBundleUrl,
        inMemory,
      }
    ...
    async generateBundle(outputOptions, bundle) {
      ...
      const entryLoaders = await Promise.all(
        Object.entries(bundle)
          ...
          .map(async ([key, chunk]) => {
            const code = await renderHotEntryLoader({
              chunk,
              serveRuntime,
              port,
              inMemory,
